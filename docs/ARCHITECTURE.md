...
- A Python porting/mirroring workspace in src/ that models command/tool/runtime concepts using snapshot-backed metadata.
- An active Rust CLI/runtime workspace in rust/ that contains executable runtime logic.
...
**Why it matters:** Trades runtime flexibility for predictability—snapshot changes require a process restart. This is acceptable for a mirroring/testing surface where registries rarely change.
...
This means these packages mostly describe archived structure, not full Python runtime behavior.
...
- Core loop: rust/crates/runtime/src/conversation.rs (ConversationRuntime.run_turn)
- Policy checks: rust/crates/runtime/src/permissions.rs (PermissionPolicy.authorize)
- Hook execution: rust/crates/runtime/src/hooks.rs (HookRunner)
- Tool catalog + execution: rust/crates/tools/src/lib.rs (mvp_tool_specs, execute_tool, GlobalToolRegistry)
- Slash command model: rust/crates/commands/src/lib.rs (SlashCommandSpec, SlashCommand.parse)
...
2. src/runtime.py PortRuntime routes prompt using command/tool metadata from src/commands.py and src/tools.py.
3. src/execution_registry.py converts matches into executable wrappers.
4. src/query_engine.py simulates turn output/usage/stream events.
5. src/session_store.py and src/transcript.py persist and compact session state.

## 7) What Is Clearly Simulated vs Fully Implemented

Clearly simulated in Python mirror:
